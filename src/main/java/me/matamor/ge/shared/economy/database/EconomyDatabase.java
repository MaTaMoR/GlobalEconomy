...
public class EconomyDatabase {
    ...
    public boolean loadDatabase() {
        try {
            ...
            this.database.openConnection().createStatement().execute("CREATE TABLE IF NOT EXISTS economy (uuid VARCHAR(36), account VARCHAR(32), double balance, primary key(uuid, account);");
            return true;
    ...
    public void save(@NotNull EconomyEntry economyEntry) throws SQLException {
        try (PreparedStatement statement = this.database.getConnection().prepareStatement("INSERT INTO economy (uuid, uuid) VALUES (?, ?) ON DUPLICATE KEY UPDATE balance = VALUES(balance);")) {
